...
import { parseSculptPlan, resolveSculptMaskMark } from '../utils/aiSculpt';
...
export const AiSculptNode: React.FC<AiSculptNodeProps> = ({
    node,
    nodes,
    onPositionChange,
    onDataChange,
    ...
    const sculptPlan = useMemo(() => {
        return parseSculptPlan(resolvedInputs.planValue);
    }, [resolvedInputs.planValue]);

    const sculptMask = useMemo(() => {
        return resolveSculptMaskMark(resolvedInputs.maskValue);
    ...
    useEffect(() => {
        const resultPayload = {
            ...
            maskMark: sculptMask,
            opsCount: sculptPlan.ops.length,
        };
        ...
        const currentSignature = JSON.stringify({
            meshSourceNodeId: (node.data.sculptResult as { meshSourceNodeId?: string | null } | undefined)?.meshSourceNodeId ?? null,
            plan: node.data.sculptPlan ?? { ops: [] },
            mask: (node.data.sculptResult as { maskMark?: unknown } | undefined)?.maskMark ?? null,
        });
        ...
        onDataChange(node.id, {
            sculptPlan,
            sculptOps: sculptPlan.ops,
            sculptResult: resultPayload,
        });
    }, [
        node.id,
        node.data.sculptPlan,
        node.data.sculptResult,
        onDataChange,
    ...
    return (
        <CustomNode
            ...
            data={{
                ...node.data,
                customName: node.data.customName || 'AI Sculpt',
