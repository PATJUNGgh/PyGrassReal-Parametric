...
const resolveSizingConfig = (nodeType: NodeData['type']): NodeSizing | undefined => {
    const configOrAlias = nodeSizeFallbacks[nodeType];
    if (!configOrAlias) return undefined;
    if (typeof configOrAlias === 'string') {
        const alias = nodeSizeFallbacks[configOrAlias];
        return (typeof alias === 'string' ? nodeSizeFallbacks[alias] : alias) as NodeSizing | undefined;
    }
    return configOrAlias;
};

const getNodeBoundingBoxFromDOM = (node: NodeData) => {
    if (typeof document === 'undefined') return null;

    const domEl = document.getElementById(node.id);
    if (!domEl) return null;

    const measuredWidth = domEl.offsetWidth;
    const measuredHeight = domEl.offsetHeight;
    if (measuredWidth <= 0 || measuredHeight <= 0) return null;

    const nodeRect = domEl.getBoundingClientRect();
    const rawScale = nodeRect.width > 0 ? nodeRect.width / measuredWidth : 1;
    const safeScale = rawScale > 0 ? rawScale : 1;

    let extraLeft = 0;
    let extraRight = 0;

    const portEls = domEl.querySelectorAll<HTMLElement>('.node-port');
    portEls.forEach((portEl) => {
        const portRect = portEl.getBoundingClientRect();
        extraLeft = Math.max(extraLeft, (nodeRect.left - portRect.left) / safeScale);
        extraRight = Math.max(extraRight, (portRect.right - nodeRect.right) / safeScale);
    });

    const clampedExtraLeft = Math.max(0, extraLeft);
    const clampedExtraRight = Math.max(0, extraRight);

    return {
        x: node.position.x - clampedExtraLeft,
        y: node.position.y,
        width: measuredWidth + clampedExtraLeft + clampedExtraRight,
        height: measuredHeight
    };
};

export const getNodeBoundingBox = (node: NodeData) => {
    // DOM-first: prefer measured size from the rendered node.
    const domBoundingBox = getNodeBoundingBoxFromDOM(node);
    if (domBoundingBox) {
        return domBoundingBox;
    }

    // Fallback: estimation for nodes that are not currently rendered.
    ...
    const isPrimitive = ['sphere', 'box', 'cone', 'cylinder', 'vector-xyz'].includes(node.type);
    if (!isPrimitive && node.data?.width && node.data?.height && node.data.width > 1 && node.data.height > 1) {
        ...
        const sizingConfig = resolveSizingConfig(node.type);
        ...
        if (sizingConfig) {
            ...
            if (sizingConfig.extraLeft !== undefined) extraLeft = sizingConfig.extraLeft;
            if (sizingConfig.extraRight !== undefined) extraRight = sizingConfig.extraRight;
        }
    ...
    let adjustedHeight = baseHeight + EXTRA_BOTTOM;
