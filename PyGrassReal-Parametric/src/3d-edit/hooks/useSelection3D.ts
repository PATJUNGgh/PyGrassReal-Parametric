    ...
    useEffect(() => {
        ...
        sceneObjects.forEach(obj => {
            if (obj.ref && obj.ref.current) {
                const rootObject = obj.ref.current as THREE.Object3D;
                if (rootObject.userData?.isSelectable === false) return;

                let parent: THREE.Object3D | null = rootObject;
                while (parent) {
                    if (!parent.visible) return;
                    parent = parent.parent;
                }

                const box3 = new THREE.Box3().setFromObject(rootObject);
