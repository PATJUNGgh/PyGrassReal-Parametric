...
export function useSelectionBoxSync({
  ...
  useFrame(() => {
    ...
    const forceAsSingleObject = target.userData?.selectionBoxAsObject === true;
    const isGroupOrMulti = !forceAsSingleObject && (isDashed || (target.type === 'Group' || target.children.length > 0));
    const finalPosition = frameState.tempBox.min.clone().set(0, 0, 0).add(target.position); // Re-use vector or clone target pos
    ...
    if (isGroupOrMulti) {
      if (target.children.length > 0 && target.children.some((c) => c.visible)) {
        ...
        if (!box.isEmpty()) {
          ...
          box.getCenter(tempBox.min); // Reuse tempBox.min as temp vector for center
          // Transform to World Space
          tempBox.min.applyMatrix4(target.matrixWorld);
          finalPosition.copy(tempBox.min);
        } else {
    ...
    handlesRef.current.position.copy(finalPosition);
