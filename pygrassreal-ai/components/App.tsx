...
import { User, LogOut, MoreVertical, Plus, Share2, Box, HardDrive, FolderOpen, Save, Menu, ChevronDown } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const [layers, setLayers] = useState<Layer[]>([
    { id: 'default', name: 'Default', color: '#ffffff', visible: true, locked: false, isCurrent: true },
    { id: 'layer01', name: 'Structure', color: '#ff3b30', visible: true, locked: false, isCurrent: false },
    { id: 'layer02', name: 'Details', color: '#007aff', visible: true, locked: false, isCurrent: false },
  ]);
  ...
  const handleLogout = async () => {
    if (!isSupabaseConfigured || isGuestMode) {
      setIsGuestMode(false);
      setUser(null);
      setCommandHistory(prev => [...prev, 'System: Exited Workspace']);
      return;
    }
  ...
  const handleGuestLogin = (mockUser: any) => {
      setIsGuestMode(true);
      setUser(mockUser); // Set the mock user as active user
      setIsAuthModalOpen(false);
      setCommandHistory(prev => [...prev, `System: Entered as ${mockUser.email}. Features unlocked.`]);
  };
    ...
    setObjects(prev => {
        if (isMultiSelect) {
            if (Array.isArray(idOrIds)) {
                 ...
                 return prev.map(obj => ({ ...obj, selected: currentSelectedIds.has(obj.id) }));
            } else {
                return prev.map(obj => {
                    if (idsToProcess.includes(obj.id)) return { ...obj, selected: !obj.selected };
                    return obj;
                });
            }
        } else {
            return prev.map(obj => ({ ...obj, selected: idsToProcess.includes(obj.id) }));
        }
    });
  };
  ...
  const handleBatchTransform = (updates: { id: string; data: Partial<IObject3D> }[]) => {
    setObjects(prev => {
        ...
        return prev.map(obj => {
            if (updatesMap.has(obj.id)) return { ...obj, ...updatesMap.get(obj.id) };
            return obj;
        });
    });
  };
  ...
  const handleCommand = (cmd: string) => {
    ...
    else if (lowerCmd === 'point') { setActiveTool('point'); setCommandHistory(prev => [...prev, 'Pick location for Point']); }
    ...
    else if (lowerCmd === 'help') setCommandHistory(prev => [...prev, 'Available: box, sphere, cylinder, point, polyline, curve, move, rotate, scale, delete, units']);
  ...
  return (
    <div className="flex flex-col h-screen bg-[#121212] overflow-hidden text-sans selection:bg-[#0078d7] selection:text-white">
      {/* 1. Menu Bar - Pro Style */}
      <div className="h-12 bg-[#181818] border-b border-[#2a2a2a] flex items-center px-4 text-xs text-gray-300 select-none relative shadow-md z-50 shrink-0">
         
         {/* App Logo */}
         <div 
           className="flex items-center gap-3 mr-6 select-none cursor-pointer group"
           onClick={() => window.location.href = 'https://www.pygrassrealai.cloud'}
           title="Back to Main Site"
         >
            <div className="w-8 h-8 bg-gradient-to-br from-[#0078d7] to-[#005a9e] rounded-md flex items-center justify-center shadow-lg border border-[#ffffff10] group-hover:scale-105 transition-transform">
               <Box size={20} className="text-white" strokeWidth={2.5} />
            </div>
            <div className="hidden md:flex flex-col leading-none">
               <span className="font-extrabold text-white text-sm tracking-wide">PyGrass<span className="text-[#3399ff]">Real</span></span>
               <span className="text-[9px] text-gray-500 font-bold uppercase tracking-widest">Professional</span>
            </div>
         </div>

         {/* Menu Items */}
         <div className="flex items-center h-8">
           {['File', 'Edit', 'View', 'Curve', 'Surface', 'Solid', 'Transform', 'Tools', 'Help'].map((item) => (
             <div 
                key={item} 
                className="relative h-full flex items-center px-3 hover:bg-[#2a2a2a] text-gray-400 hover:text-white cursor-pointer transition-colors rounded-sm mx-0.5"
                onClick={item === 'File' ? () => setIsFileMenuOpen(!isFileMenuOpen) : undefined}
             >
           ...
           {/* File Dropdown */}
           {isFileMenuOpen && (
              <div className="absolute top-10 left-24 mt-1 w-56 bg-[#1e1e1e] border border-[#333] shadow-2xl rounded-md py-1 z-50 animate-in fade-in zoom-in-95 duration-100 ring-1 ring-white/5">
                 <div className="px-3 py-2 border-b border-[#333] mb-1">
                     <span className="text-[10px] text-gray-500 font-bold uppercase tracking-wider">Project Actions</span>
                 </div>
                 <div className="group px-3 py-2 hover:bg-[#0078d7] cursor-pointer flex items-center gap-3" onClick={() => { setIsFileMenuOpen(false); createNewObject('box'); }}>
                   <Plus size={14} className="text-gray-400 group-hover:text-white" /> 
                   <span className="text-gray-200 group-hover:text-white">New Project</span>
                 </div>
                 <div className="group px-3 py-2 hover:bg-[#0078d7] cursor-pointer flex items-center gap-3" onClick={() => { setIsFileMenuOpen(false); handleTriggerLoad(); }}>
                   <FolderOpen size={14} className="text-gray-400 group-hover:text-white" /> 
                   <span className="text-gray-200 group-hover:text-white">Open...</span>
                 </div>
                 <div className="group px-3 py-2 hover:bg-[#0078d7] cursor-pointer flex items-center gap-3" onClick={() => { setIsFileMenuOpen(false); handleTriggerSave(); }}>
                   <Save size={14} className="text-gray-400 group-hover:text-white" /> 
                   <span className="text-gray-200 group-hover:text-white">Save</span>
                 </div>
              </div>
           )}
         </div>
         ...
         <div className="ml-auto flex items-center gap-4">
            {currentProjectId && (
              <div className="flex flex-col items-end justify-center">
                 <span className="text-[9px] text-gray-600 uppercase tracking-wider font-extrabold">Current Project</span>
                 <span className="text-xs text-gray-300 font-semibold">{currentProjectName}</span>
              </div>
            )}
            
            {isGuestMode && (
              <div className="flex items-center gap-2 bg-yellow-500/10 text-yellow-500 px-3 py-1.5 rounded-full border border-yellow-500/20 text-[10px] font-bold">
                 <HardDrive size={12} />
                 GUEST MODE
              </div>
            )}
            ...
            <button 
               ...
               className={`bg-[#0078d7] hover:bg-[#0060ac] text-white px-3 py-1.5 rounded text-xs flex items-center gap-2 transition-colors font-bold shadow-lg shadow-blue-900/20`}
            >
               <Share2 size={12} />
            ...
            {authLoading ? (
              <span className="text-gray-600 italic text-[10px]">Loading...</span>
            ) : (user || isGuestMode) ? (
              <div className="flex items-center gap-3">
                <div className="flex items-center gap-2 text-gray-300 bg-[#252525] px-3 py-1.5 rounded-full border border-[#333] transition-colors hover:border-[#555] cursor-pointer group">
                   <User size={14} className="text-[#0078d7]" />
                   <span className="max-w-[120px] truncate font-medium text-xs">
                      {user ? (userProfile?.full_name || user.email || user.user_metadata?.full_name) : 'Guest User'}
                   </span>
                   <ChevronDown size={10} className="text-gray-500 group-hover:text-white transition-colors" />
                </div>
                <button 
                  onClick={handleLogout}
                  className="text-gray-500 hover:text-red-400 p-1.5 rounded hover:bg-[#2a2a2a] transition-colors"
              ...
              <button 
                ...
                className="flex items-center gap-2 text-gray-300 hover:text-white px-3 py-1.5 rounded hover:bg-[#2a2a2a] transition-colors text-xs font-bold"
              >
                Log In / Sign Up
              </button>
            )}
      ...
      {/* 2. Command Area - Terminal Style */}
      <CommandBar history={commandHistory} onCommand={handleCommand} />

      {/* 3. Main Workspace */}
      <div className="flex-1 flex flex-row min-w-0 bg-[#121212] relative overflow-hidden">
        ...
        <div className="flex-1 flex flex-col min-w-0 bg-[#121212] relative border-l border-r border-[#2a2a2a]">
          ...
          >
            {maximizedViewport ? (
              ...
              <div 
                ref={gridRef}
                className="absolute inset-0 grid gap-[2px] bg-[#000]"
                style={{ 
              ...
              >
                ...
                {/* Drag Handles */}
                <div 
                  className="absolute top-0 bottom-0 w-1 -ml-0.5 cursor-col-resize z-50 hover:bg-[#0078d7] bg-[#2a2a2a] hover:w-1.5 transition-all"
                  style={{ left: `${gridSplit.x}%` }}
                  onMouseDown={() => setDragState({ type: 'x' })}
                />
                <div 
                  className="absolute left-0 right-0 h-1 -mt-0.5 cursor-row-resize z-50 hover:bg-[#0078d7] bg-[#2a2a2a] hover:h-1.5 transition-all"
                  style={{ top: `${gridSplit.y}%` }}
                  onMouseDown={() => setDragState({ type: 'y' })}
                />
                <div 
                  className="absolute w-6 h-6 -ml-3 -mt-3 bg-[#2a2a2a] border border-[#444] rounded-full shadow-xl cursor-move z-50 flex items-center justify-center hover:bg-[#0078d7] hover:text-white hover:scale-110 transition-all"
                  style={{ left: `${gridSplit.x}%`, top: `${gridSplit.y}%` }}
                  onMouseDown={() => setDragState({ type: 'center' })}
                >
                  <MoreVertical size={12} className="text-gray-400 hover:text-white" />
                </div>
              </div>
            )}
          </div>

          {/* Bottom Tabs - Clean Look */}
          <div className="h-8 bg-[#181818] border-t border-[#2a2a2a] flex items-center px-0 text-[11px] font-semibold select-none">
             {['Perspective', 'Top', 'Front', 'Right'].map((view) => (
               <button 
                 key={view}
                 className={`px-5 h-full border-r border-[#2a2a2a] flex items-center justify-center transition-all ${
                   maximizedViewport === view 
                     ? 'bg-[#0078d7] text-white shadow-inner' 
                     : 'text-gray-500 hover:bg-[#222] hover:text-white'
                 }`}
                 onClick={() => handleTabClick(view as ViewportType)}
               >
                 {view}
               </button>
             ))}
             ...
             <button 
               className="px-3 h-full border-r border-[#2a2a2a] text-gray-600 hover:bg-[#222] hover:text-white flex items-center justify-center transition-colors"
               onClick={() => {}}
               title="New Layout"
             >
               <Plus size={12} />
             ...
             {maximizedViewport && (
               <button
                 className="ml-auto mr-3 text-gray-500 hover:text-[#0078d7] text-[10px] flex items-center gap-1.5 transition-colors"
                 onClick={() => setMaximizedViewport(null)}
               >
                 <GridIcon size={12} /> 
                 <span className="font-bold">RESTORE 4-VIEW</span>
               </button>
             )}
          </div>
      ...
      <AuthModal 
        ...
        onContinueGuest={handleGuestLogin}
      />
      <ShareModal 
